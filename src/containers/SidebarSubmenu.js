...
import SidebarChildSubmenu from './SiderbarChildSubmenu'
import { icons } from "../routes/sidebar"

function SidebarSubmenu({ submenu, name, icon, iconName }) {
        ...
        <ul className={`menu menu-compact`}>
          {
            submenu.map((m, k) => {
              return (
                <li key={k}>
                  {
                    m.submenu && m.submenu?.length > 0 ? <SidebarChildSubmenu {...m} /> : (
                      <Link to={m.path}>
                        {m.icon ? m.icon : icons[iconName]} {m.name}
                        {
